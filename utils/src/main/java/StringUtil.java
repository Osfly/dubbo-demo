...
public class StringUtil {
	 ...
	 * 
	 * <pre>
	 * StringUtil.format("aaa{0}def{1}bbb{2}", 111, "222", "333") = "aaa111def222bbb333"
	 * </pre>
	 * <p>
	 * 如果字符串是<code>null</code>则返回<code>null</code>。
	 * 
	 * <pre>
	 * StringUtil.format(null)  = null
	 * StringUtil.format("")    = ""
	 * </pre>
	 * 
	 * 如果未提供替换字符，也不做替换;
	 * 
	 * <pre>
	 * StringUtil.format("aaaa") = "aaaa"
	 * StringUtil.format("aaa{0}def") = "aaa{0}def"
	 * </pre>
	 * 
	 * 如果有相同占位，则重复替换
	 * 
	 * <pre>
	 * StringUtil.format("aaa{0}def{1}bbb{0}", 111, "222") = "aaa111def222bbb111"
	 * </pre>
	 * </p>
	 * 
	 * @param str  要替换的字符创
	...
	public static String format(String str, Object... args) {
		if (args == null || args.length == 0 || isEmpty(str))
			return str;
		for (int i = 0; i < args.length; i++) {
			str = str.replace("{" + i + "}", args[i] == null ? "" : args[i].toString());
		}
		return str;
	}
	...
	/**
	 * 进行字符串替换，将字符串指定位数的字符替换为指定的符号
	 * StringUtil.replace("13699069604",3,5,'*')=136***69604;
	 * 
	 * @param input 原始字串
	 * @param start 开始位置
	 * @param end   结束位置
	 * @param sign  符号
	 */
	public static String replace(String input, int start, int end, char sign) {
		...
		String s2 = input.substring(end + 1, input.length());
		input = s1;
	 ...
	 * @return
	 */
	public static String htmlFilterJsAndCss(String inputString) {
	...
	public static void main(String[] args) {
		...
		htmlStr.append(
				...
				.append("<html xmlns='http://www.w3.org/1999/xhtml' xml:lang='en'><head><title>aaa</title><script type='text/javascript'></script>")
				.append("<link href='static_files/help.css' mce_href='static_files/help.css' rel='stylesheet' type='text/css' media='all' />")
				.append("</head><body><ul><li>XXXX</li></ul><p>ppp</p><img src='d:\tt.jpg'></img></body></html>");

		System.out.println(htmlFilterJsAndCss(htmlStr.toString()));
		
		String string = "click: {0}";
		String[] vStrings = {"function(d){\n" + 
				"		  	var treeData = d.current;\n" + 
				"		  	var ev = '2'.split(',');\n" + 
				"		  	if(ev.indexOf(treeData['id'])>-1){\n" + 
				"		      	layui.treeSelect.checkNode('', );\n" + 
				"		  	  	layer.msg('上级组织不能选择自己!', {icon: 2});\n" + 
				"		  	}else{\n" + 
				"		  	  	$('#').val(treeData['id']);\n" + 
				"		  	}  }"};
		for (int i = 0; i < vStrings.length; i++) {
//			string = string.replace("{" + i + "}", vStrings[i] == null ? "" : vStrings[i].toString());
			string = string.replaceAll("({" + i + "})", vStrings[i] == null ? "" : vStrings[i].toString());
		}
		System.out.println(string);
	}

	/**
	 * 将文本信息按指定格式构造成Map对象 input 格式为：“key1=value1,key2=value1.....”
	 * 
	 * @param input
	 * @param split 分隔符,隔开符可以为任意的char类型，但key与value之间必须是=号
	 * @return
	 */
	public static Map<String, String> formatToMap(String input, char split) {
		Map<String, String> map = new HashMap<String, String>();
		if (isEmpty(input)) {
			return map;
		}
		String[] texts = StringUtil.split(input, split);
		for (String text : texts) {
			if (text.indexOf("=") == -1) {
				continue;
			}
			map.put(StringUtil.split(text, '=')[0], StringUtil.split(text, '=')[1]);
		}
		return map;
	}
	...
	public static String toString(Object obj, String defalutValue) {
		return null == obj ? defalutValue : String.valueOf(obj);
	}
